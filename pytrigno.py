...
#--------------- Class Inertial Measurement (quaternions -> angle of inclination) --------------------
class IM(_BaseTrignoDaq):
    """    Delsys Trigno wireless IMU system — orientation data via quaternions.
 
    Reads AUX data (port 50044) in orientation mode (e.g. mode 39 for
    an Avanti sensor: EMG + 5-channel orientation).
 
    According to the SDK, in orientation mode each sensor exposes 5 channels:
        - channel 0: EMG (port 50043, ignored here)
        - channels 1-4: Quaternion w, x, y, z (port 50044, AUX)
 
    The AUX port (50044) supports a total of 144 channels (9 AUX channels x 16 slots).
    Each sensor occupies 9 positions in the AUX buffer.
    The first 4 of these 9 channels are the w, x, y, z components of the quaternion.
 
    Parameters
    ----------
    channel_range: tuple (int, int)
        Start and end indices (inclusive) in the AUX buffer of the sensors to be read.
        For the sensor in slot 1: (0, 3) → w, x, y, z.
        For the sensor in slot 2: (9, 12), etc.
        Formula: slot n (base 1) → (9*(n-1), 9*(n-1)+3).
    samples_per_read : int
        Number of samples to read per call to read().
    sensor_slot : int
        TCU slot number of the sensor (1-16). Used to configure the mode
        and automatically calculate channel_range if not provided.
    host : str, optional
        IP address of the TCU server.
    cmd_port: int, optional
        TCU command port.
    data_port: int, optional
        AUX data port (50044 by default).
    ...
    ----------
    rate: float
    Quaternion sampling rate (~74 Hz in mode 39).
    """
    def __init__(self, sensor_slot=1, samples_per_read=1, channel_range=None, host='localhost',cmd_port=50040, data_port=50044, timeout=10):
        
        super(IM, self).__init__(
            host=host, 
            cmd_port=cmd_port, 
            data_port=data_port,
            total_channels = 144, #9 channels AUX x 16 slots
            timeout = timeout
            )
        ...
        if channel_range is not None : 
            ...
            offset = 9 * (sensor_slot - 1) 
            self.channel_range = (offset, offset + 3)

        self.rate = 74.0
    
    def set_mode(self, mode):
        """
        Configure the sensor mode via the TCU control port.
 
        For quaternions, use an “Orientation” mode:
            - Mode 39: EMG (1481 Hz) + Orientation 74 Hz, 32-bit — 1 slot
 
        Parameters
        ----------
        mode : int
            Mode number to apply to the sensor (see SDK, section 7.1).
        """
        self.mode = mode
        cmd = f"SENSOR {self.sensor_slot} SETMODE {mode}"
        self._send_cmd(cmd)

    def read(self):
        """
        Reads an AUX data block and returns the raw quaternions.
 
        Returns
        -------
        data : ndarray, shape=(4, samples_per_read)
            [w, x, y, z] quaternions for each sample.
            The values are normalised (between -1 and 1).
        """
        data = super(IM, self).read(self.samples_per_read)
    ...
    def start(self):
        """
        Starts the streaming of the IMU data
        """
        super(IM, self).start()
    
    def stop(self):
        """
        Stop straming the IMU data
        """
        super(IM, self).stop()
    ...
    def quaternion_to_euler(w, x, y, z):
        """
        Converts a quaternion (w, x, y, z) into Euler angles in degrees
        Convention ZYX (yaw -> pitch -> roll), standard for the IMU
        
        Parameters 
        ----------
        w, x, y, z : float or ndarray
            Components of the quaternion. Can be scalars 
            or numpy arrays (a sample or a batch of samples)
        ...
        Notes
        -----
        The formulas used are the standard analytical conversions
        quaternion → rotation matrices → Euler angles ZYX.
            
        """

        #This value designed to prevent the propagation of NaN values when calculating the norm of quaternion at sensor start-up
        epsilon = sys.float_info.min
        norm = numpy.sqrt(w**2 + x**2 + y**2 + z**2) + epsilon
        ...
        cos_r_cos_p = 1.0 - 2.0 * (x**2 + y**2)
        roll = numpy.degrees(numpy.arctan2(sin_r_cos_p, cos_r_cos_p))

        #--- Pitch (rotation about the Y-axis) ---
        sin_p = 2.0 * (w * y - z * x)
        sin_p = numpy.clip(sin_p, -1.0, 1.0)
        pitch = numpy.degrees(numpy.arcsin(sin_p))

        # # --- Yaw (rotation around the Z-axis) ---
        # sin_y_cos_p = 2.0 * (w * z + x * y)
        # cos_y_cos_p = 1.0 - 2.0 * (y**2 + z**2)
        # yaw = numpy.degrees(numpy.arctan2(sin_y_cos_p, cos_y_cos_p))

        return roll, pitch#, yaw
    
    def read_euler(self):
        """
        Reads a block of data and returns the Euler angles directly.
 
        Returns
        -------
        roll : ndarray, shape=(samples_per_read,)
            Roll angle in degrees.
        pitch : ndarray, shape=(samples_per_read,)
            Pitch angle in degrees.
        yaw : ndarray, shape=(samples_per_read,)
            Yaw angle in degrees.
        """
        quat = self.read()   # shape (4, samples_per_read) → [w, x, y, z]
        w = quat[0, :]
        x = quat[1, :]
        y = quat[2, :]
        z = quat[3, :]
        return self.quaternion_to_euler(w, x, y, z)
    
    def prepare_buffer(self):
        """
        Wait until the IMU sensor starts streaming non-zero data.
